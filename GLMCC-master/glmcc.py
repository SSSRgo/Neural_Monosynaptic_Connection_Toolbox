...
def calc_grad_log_p(par, beta, tau, c, n_sp, t_sp, delay_synapse, Gk):
    ...
    ####### Rui Shen updated start #######
    t_sp = np.array(t_sp)
    tau = np.array(tau)

    delay_synapse_pos = delay_synapse
    delay_synapse_neg = -delay_synapse

    condition_pos = t_sp > delay_synapse_pos
    condition_neg = t_sp < delay_synapse_neg


    tmp_ij= np.sum(np.exp(-(t_sp[condition_pos] - delay_synapse_pos) / tau[0]))
    tmp_ji= np.sum(np.exp(-(np.abs(t_sp[condition_neg]) - delay_synapse_pos) / tau[1]))
    ####### Rui Shen updated end #######
...
def GLMCC(c, t_sp, tau, beta, pre, post, delay_synapse, cond = 0):
    ...
    ####### Rui Shen updated start #######
    # Convert t_sp to a NumPy array
    t_sp_array = np.array(t_sp)
    np.array(range(0,100,1))
    # Initialize new_c as a NumPy array
    new_c = np.zeros(NPAR)

    k = (t_sp_array + WIN) / DELTA
    tmp = np.floor(k).astype(int)

    # Handle the k - tmp == 0 condition
    equal_indices = np.where(k - tmp == 0)[0]

    equal_add = np.histogram(tmp[equal_indices], np.array(range(0, NPAR+1, 1)))
    new_c += equal_add[0]/2

    equal_add2 = np.histogram(tmp[equal_indices]-1, np.array(range(0, NPAR+1, 1)))
    new_c += equal_add2[0]/2

    # Handle the general case 0 <= tmp < NPAR-2
    valid_indices = np.where((k - tmp != 0) & (tmp >= 0) & (tmp < NPAR - 2))[0]
    valid_add = np.histogram(tmp[valid_indices], np.array(range(0, NPAR+1, 1)))
    new_c += valid_add[0]
    # Check for errors
    error_indices = np.where((tmp < 0) | (tmp >= NPAR - 2))[0]
    for index in error_indices:
        print('Error: ' + str(t_sp_array[index]))

    # Handle delay_synapse conditions
    synapse_delay_indices = np.where(t_sp_array > delay_synapse)[0]
    new_c[NPAR - 2] = np.sum(np.exp((-1) * (t_sp_array[synapse_delay_indices] - delay_synapse) / tau[0]))

    negative_synapse_delay_indices = np.where(t_sp_array < -delay_synapse)[0]
    new_c[NPAR - 1] = np.sum(np.exp((t_sp_array[negative_synapse_delay_indices] + delay_synapse) / tau[1]))
    ####### Rui Shen updated end #######
